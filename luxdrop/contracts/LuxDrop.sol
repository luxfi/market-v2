...
// bridge needs to be able to burn and mint tokens on different chains

// function bridgeMint(uint256 quantity) public payable nonReentrant {
//address sender = msg.sender;
// 
//}

contract LuxDrop is ERC721A, Ownable, ReentrancyGuard {
    ...
    // List of all token URIs
    string[] private tokenURIs;

    // List of index for each drop based on total supply changes
    uint256[] private dropIndexes;
    ...
    // function multiMint(tuple(uint256, uint265)[] drops) public payable nonReentrant {
    //     for (i=0; i<drops.len(); i++) {
    //         let (uriIndex, quantity) = drops[i];
    //         _currentId = currentId + uriIndex << 128;
    //         _safeMint(quantity)
    //         _currentId = currentId & (1 << (129 - 1))
    //     }
    // }
    ...
    function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
        ...
        for (uint256 i = 0; i<dropIndexes.length; i++) {
            if (tokenId <= dropIndexes[i]) {
                return tokenURIs[i];
            }
        }
    }
    ...
    function addDrop(string memory tokenURI, uint256 quantity) public  {
        tokenURIs.push(tokenURI);
        _safeMint(msg.sender, quantity);

        totalSupply_ += quantity;
        dropIndexes.push(totalSupply_);
    }
}

    // function tokenID(uint128 UriIndex) (uint256 tokenId) {
    //     uint256 tokenId = UriIndex << 128 + _currentId;
    // }

    // function tokenURI(uint256 tokenId) public view (string) {
    //     return tokenUris[tokenId >> 128];
    // }

    // function uriIndex(string uri) public view (uint256) {
    //     return tokenUris.find(uri);
    // }



    // function multiMint(tuple(uint256, uint265)[] drops) {
    //     // should loop in assembly
    //     for (i = 0; i < drops.len(); i++) {
    //         (uint256 uriIndex, uint256 quantity) = drops[i];
    //         _currentId += uriIndex << 128;
    //         _safeMint(quantity)
    //         _currentId -= uriIndex << 128;
    //     }
    // }
